...
def rbf(x_1, x_2, sigma=1.):
    ...
    ''' 
    distances = np.zeros((x_1.shape[0], x_2.shape[0]))
    for i in range(x_1.shape[0]):
        distances[i,:] = torch.sum( (x_2[:]-x_1[i])**2 , axis = 1)
...
def hinge_loss(scores, labels):
    ...
    return torch.sum(torch.maximum(torch.zeros(scores.shape),1-scores*labels))/scores.shape[0]
...
class SVM(BaseEstimator, ClassifierMixin):
    ...
    def predict_scores(self, batch):
        with torch.no_grad():
            ...
            return torch.reshape(K@self.betas + self.bias, (-1,) ) ### YOUR CODE HERE
